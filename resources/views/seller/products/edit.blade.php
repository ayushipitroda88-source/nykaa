    ...
    <div class="col-md-8 mx-auto">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white pb-0 pt-4">
                <h4 class="mb-0">Edit Product: {{ $product->title }}</h4>
            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="main_category" class="form-select">
                                ...
                                @foreach($mainCategories as $category)
                                    <option value="{{ $category->id }}" {{ ($mainCategory && $mainCategory->id == $category->id) ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
...
$(document).ready(function() {
    ...
    @if($mainCategory)
        loadSubCategories({{ $mainCategory->id }}, {{ $subCategory ? $subCategory->id : 'null' }}, {{ $childCategory ? $childCategory->id : 'null' }});
    @endif
});
</script>
