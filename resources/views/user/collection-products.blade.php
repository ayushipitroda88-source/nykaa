...
.collection-hero {
    background: linear-gradient(135deg, #fc2779 0%, #ff5ba8 60%, #ffb3d1 100%);
    border-radius: 20px;
    padding: 36px 40px;
    margin-bottom: 28px;
    color: #fff;
    ...
    position: relative;
    overflow: hidden;
    box-shadow: 0 12px 40px rgba(252,39,121,.25);
}
...
.collection-hero-info { position: relative; z-index: 1; }
...
.products-count { color: #888; font-size: 13px; font-weight: 500; }
.sort-dropdown { display: flex; align-items: center; gap: 8px; }
.sort-dropdown label { font-size: 13px; font-weight: 600; color: #666; white-space: nowrap; }
...
.price-current { font-size: 20px; font-weight: 800; color: #fc2779; }
.price-old { font-size: 13px; color: #bbb; text-decoration: line-through; }
.price-save {
    font-size: 11px; font-weight: 700; color: #2ecc71;
    background: #eafaf1; padding: 2px 8px; border-radius: 20px;
}
...
.cart-btn {
    flex: 1; padding: 9px 10px;
    background: linear-gradient(135deg, #fc2779, #ff5ba8); color: #fff;
    border: none; border-radius: 10px;
    font-size: 13px; font-weight: 600;
...
<div class="collection-page">
    ...
    <div style="margin-bottom: 20px;">
        ...
        <div class="breadcrumb-box">
            <a href="{{ url('/') }}">Home</a> / <a href="#">Collections</a> / <strong>{{ $collection->name }}</strong>
        </div>
    </div>
    ...
    <div class="collection-hero">
        <div class="collection-hero-info">
            ...
            <p>{{ $collection->description ?? 'Explore our exclusive curated collection' }}</p>
            @if($collection->discount > 0 && $collection->discount_start && $collection->discount_end && now()->between($collection->discount_start, $collection->discount_end))
                <div style="margin-top:12px;">
                    <span style="background:rgba(255,255,255,.25);border:1px solid rgba(255,255,255,.4);padding:6px 18px;border-radius:30px;font-weight:700;font-size:14px;">
                        🎉 {{ $collection->discount }}% OFF on all items
                    </span>
                </div>
            @endif
        </div>
        <div class="collection-hero-badge">
            🛍️ {{ $products->total() }} Products
        </div>
    </div>
    ...
    <div class="collection-wrapper">
        ...
        <div class="collection-products">
            ...
            @php
                $hasCollDiscount = $collection->discount > 0
                    && $collection->discount_start
                    && $collection->discount_end
                    && now()->between($collection->discount_start, $collection->discount_end);
                $collDiscPct     = $hasCollDiscount ? $collection->discount : 0;
                $hasOldPrice     = $product->old_price && $product->old_price > $product->price;
                $effectiveDisc   = $hasCollDiscount ? $collDiscPct : ($hasOldPrice ? round((($product->old_price - $product->price) / $product->old_price) * 100) : 0);
                $finalPrice      = $hasCollDiscount ? ($product->price - ($product->price * $collDiscPct / 100)) : $product->price;
            ...
            <div class="product-card">
                ...
                @if($effectiveDisc > 0)
                    <span class="discount-badge">{{ $effectiveDisc }}% OFF</span>
                @endif
                ...
                <div class="product-info">
                    ...
                    <div class="price-box">
                        ...
                        @if($hasCollDiscount || $hasOldPrice)
                            <span class="price-old">₹{{ number_format($product->price, 0) }}</span>
                            @if($effectiveDisc > 0)
                                <span class="price-save">{{ $effectiveDisc }}% OFF</span>
                            @endif
                        @endif
                    </div>
                    ...
                    <div class="product-actions">
                        <a href="{{ route('product.show', $product->id) }}?collection_id={{ $collection->id }}" class="view-btn">
            ...
            <div class="empty-products">
                ...
                <p style="color:#999;">No products available in this collection.</p>
            </div>
            @endforelse
